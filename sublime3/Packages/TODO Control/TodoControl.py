...
def on_tags_change():
    global TAGS
    TAGS = ["%s%s%s" % (PREFIX, t, POSTFIX) for t in settings.get("tags", [])]
...
def get_tags_text(lines, tags):
    return [s for s in [get_tags(ss, tags) for ss in enumerate(lines)] if s != None]


def format_lines_tags(lines, tags):
    return ["line :%d: %s" % (i_s[0], i_s[1]) for i_s in get_tags_text(lines, tags)]
...
def get_tags_files(names, tags):
    return join_lists([get_tags_file(name, tags) for name in names])


def get_tags_view(view, tags):
    reg = sublime.Region(0, view.size())
    lines = list(map(view.substr, view.split_by_newlines(reg)))
